...
121764, 164933, 160080, 1823, 164940, 164934, 164935, 159615, 160288, 1855, 164947,162549,162877,160596,1109,1113,162309,1729,162737,159615,1120,163309,164936,1123,1124,1125,164937,1126,166607,159356,161011,165911,167161,165086,164946,165287,164941)
where e.voided=0
	...
	BEGIN
        ...
        if(max(o.date_created) > min(e.date_created),max(o.date_created),NULL) as date_last_modified
			from encounter e
				inner join person p on p.person_id=e.patient_id and p.voided=0
				inner join obs o on e.encounter_id = o.encounter_id and o.voided =0
														and o.concept_id in(1282,159922,984,1418,1425,5088,5087,5085,5086,5242,5092,5089,5090,1343,21,163590,5245,1438,1439,160090,162089,1440,162107,5356,5497,856,1305,1147,159427,164848,161557,1436,1109,
														                    5576,128256,1875,159734,161438,161439,161440,161441,161442,161444,161443,162106,162101,162096,299,159918,32,119481,165099,120198,374,161074,1659,164934,163589,165040,166665,162747,
														                    1912,160481,163145,5096,159395,163784,1271,159853,165302,1592,1591,2031577,1592,161595,299,2031735,163783,163530,160080,5624,1427,5596,1438,167958,1000443,164359)
				inner join
				(
  ...
  BEGIN
    ...
    BEGIN
        ...
        from encounter e
                 ...
                 inner join obs o on o.encounter_id = e.encounter_id and o.concept_id in (167093,1710,159427,160554,164855,159599,162053,5096,165239,161550,856,1305,165241,
    ...
    from (
             select
                 ...
                 max(if(concept_id=984 , (case when value_coded=84879 then 'Tetanus Toxoid' when value_coded=159610 then 'Malarial prophylaxis' when value_coded=104677 then 'Folate/iron'
                                               when value_coded=79413 then 'Deworming (Mebendazole 500mgs)' when value_coded=160428 then 'Long-lasting insecticidal net'
    ...
    from encounter e
             ...
             left outer join obs o on o.encounter_id = e.encounter_id and o.concept_id in
                                                                          (164181, 161643, 164448, 163145, 165302, 164204,
                                                                           ...
                                                                           165241,1000494,164209,165430,162747,1000088,162737,166663, 5219,159402,985,1151,1069,165531,168146,159893,163300,5272,160348,2031533,160433)
        and o.voided = 0
    where e.voided = 0
    group by e.patient_id, e.encounter_id;
